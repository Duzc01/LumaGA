    ...
    fn new(slice: &[u8]) -> Self {
        ...
        if slice.len() <= CUTOFF {
            ...
            unsafe {
                ...
                // Check the pointer value itself: reading `data[SZ - 1]` tests
                // the byte at the OPPOSITE end of the pointer on little-endian
                // targets, which wrongly rejects Android's top-byte-tagged
                // heap pointers (e.g. 0xb4...) even though the allocation is
                // 8-aligned.
                #[cfg(not(miri))]
