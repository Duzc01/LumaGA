...
impl Inner {
    ...
    fn new(
            ...
            && fixed_value_length.is_some()
        {
            ...
            // Compare in u64 so the wide shifts don't overflow on 32-bit
            // targets (i686), where usize is 32 bits and `1 << 32` is a
            // const-eval overflow.
            let bytes_per_offset: u8 = match max_indexable_offset as u64 {
                i if i < 256 => 1,
                i if i < (1u64 << 16) => 2,
                i if i < (1u64 << 24) => 3,
                i if i < (1u64 << 32) => 4,
