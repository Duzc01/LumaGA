...
mod test {
    ...
    async fn test_extract_error_shapes() {
        // NGA may carry errors either as `<item>` children or as bare text;
        // neither shape may panic, and the message must surface.
        let package = sxd_document::parser::parse(
            r#"<?xml version="1.0"?><root><__MESSAGE><item>1</item><item>未登录</item><item>403</item></__MESSAGE></root>"#,
        )
        .unwrap();
        match crate::utils::extract_error(&package) {
            Err(crate::error::ServiceError::Nga(e)) => {
                assert_eq!(e.code, "1");
                assert_eq!(e.info, "未登录");
            }
            other => panic!("unexpected: {other:?}"),
        }

        let package = sxd_document::parser::parse(
            r#"<?xml version="1.0"?><root><error>some direct text error</error></root>"#,
        )
        .unwrap();
        match crate::utils::extract_error(&package) {
            Err(crate::error::ServiceError::Nga(e)) => {
                assert_eq!(e.code, "?");
                assert_eq!(e.info, "some direct text error");
            }
            other => panic!("unexpected: {other:?}"),
        }

        let package = sxd_document::parser::parse(
            r#"<?xml version="1.0"?><root><__MESSAGE>direct text</__MESSAGE></root>"#,
        )
        .unwrap();
        match crate::utils::extract_error(&package) {
            Err(crate::error::ServiceError::Nga(e)) => assert_eq!(e.info, "direct text"),
            other => panic!("unexpected: {other:?}"),
        }
    }
